...
// TODO: line_coord_groups() should probably be a static method.
// TODO: the 4 argument should be taken from command-line args.
def all_possible_grid_lines = script_grid_util.line_coord_groups(number_grid.size, number_grid[0].size, 4)

// a few useful little closures
def multiplynums = { x,y -> x*y }
def lookup = { grid, coords -> grid[coords[0]][coords[1]] }
def line_product = { grid, coord_list -> coord_list.collect { lookup(grid, it) }.inject(multiplynums) }
...
println "max line-product is hopefully: " + all_possible_grid_lines.collect { line_product(number_grid, it) }.max()