...
describe 'multiples_of' do
  ...
  it 'should return all multiples of a given number up to (and including) the specified limit' do
    ...
    expect(Number_util.multiples_of(2, 10)).to match_array(my_multiples)

    my_multiples = [6,9,12,15]
    expect(Number_util.multiples_of(3, 16)).to match_array(my_multiples)
  end
end